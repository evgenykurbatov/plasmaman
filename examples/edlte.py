...
fig.suptitle(r"$N_\mathrm{i} = %g$ cm$^{-3}$" % N_i)
...
ax_ = ax[0,1]
ax_.loglog(T, ionization_energy_1*transition_fb(T, 1), label=r"$h \nu_1 \alpha$")
ax_.loglog(T, cooling_fb(T, 1), label=r"$\beta$")
...
ax_.loglog(T, section_ff_P, label=r"$\sigma_\mathrm{P}^\mathrm{ff}$")
ax_.loglog(T, section_ff_R, label=r"$\sigma_\mathrm{R}^\mathrm{ff}$")
